...
export default function OffreOptions() {
  ...
  const [installOption, setInstallOption] = useSessionState<string>(
    `pb-offre-${accountNumber}-${offerPosition}-installOption`,
    "phone"
  );
  ...
  return (
    <div>
      ...
      <div style={{ borderBottom: "1px solid var(--pb-border)" }}>
        ...
        <p className="pb-discount-line" style={{ textAlign: "right" }}>{offer.discount || "50%"} de réduction les 12 premiers mois</p>
        </div>
      </div>
      ...
      <div className="pb-main">
        ...
        <div style={{ maxWidth: "596px", margin: "0 auto", display: "flex", flexDirection: "column", gap: "24px" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "48px", paddingTop: "24px" }}>
            ...
            <Link to={`/offre/${accountNumber}/informations?offre=${offerPosition}`} className="pb-btn pb-btn-primary" style={{ padding: "12px 32px", fontSize: "16px", textDecoration: "none" }}>
              Étape suivante
            </Link>
          </div>
        </div>
